                ...
                for m, reaction in zip(self.mol, self.is_reaction_list):
                    if not reaction:
                        if m is not None and m.GetNumHeavyAtoms() > 0:
                            self.features.extend(features_generator(m, self.row))
                        # for H2
                            ...
                            self.features.extend(np.zeros(len(features_generator(Chem.MolFromSmiles('C'), self.row))))                           
                    else:
                        if m[0] is not None and m[1] is not None and m[0].GetNumHeavyAtoms() > 0:
                            self.features.extend(features_generator(m[0], self.row))
