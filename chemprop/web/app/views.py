...
def home():
    ...
    return render_template('home.html',  users={'username': 'DEFAULT', 'preferences': None})
...
def predict():
    ...
    membrane = request.form['textMembrane']
    solvent = request.form['textSolvent']
    configuration = request.form['textConfiguration']
    ...
    #model_paths =   [os.path.join(app.config['CHECKPOINT_FOLDER'], f'{model["id"]}.pt') for model in models] # TODO model path
    
    #task_names = load_task_names(model_paths[0])
    #num_tasks = len(task_names)
    #gpu = request.form.get('gpu')
    #train_args = load_args(model_paths[0]) # TODO

    # The feature_generator creates the input features coming from the user. 
    feature_generator = InputGenerator(smiles, [membrane], [solvent], configuration)
    # These are hardcoded now:
    # ['DM300'], ['Acetonitrile'], 'NF' 
    # but they needs to be changed to user input.

    feature_generator.generate_features()
    feature_generator.dump(
        rf"temp_generated_features.csv"
        ) 
    # writes a temporary .csv file
    # it will overwrite it on the next run.
    # We do not really need this file later.

    # Build arguments TODO arguments
    arguments = [
        ...
        '--features_generator','custom',   # refers to the fn here: custom_features_generator

        #'--test_path', 'None',
        '--preds_path',  'temp.csv' # os.path.join(app.config['TEMP_FOLDER'], app.config['PREDICTIONS_FILENAME']),
    ...
    # Handle additional features TODO
    # if train_args.features_path is not None:
    #     # TODO later: make it possible to specify the features generator if trained using features_path
    #     arguments += [
    ...
    preds = [pred if pred is not None else [invalid_smiles_warning] * 1 for pred in preds]
