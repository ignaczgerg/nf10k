...
# from single_input_generator import single_input_generator
from membrane_feature_generator import membrane_feature_selector
import numpy as np
...
@register_features_generator('custom')
def custom_features_generator(mol: Molecule, row) -> np.ndarray:
    # If you want to use the SMILES string
    smiles = Chem.MolToSmiles(mol, isomericSmiles=True) if type(mol) != str else mol
    # If you want to use the RDKit molecule
    mol = Chem.MolFromSmiles(mol) if type(mol) == str else mol
    features = membrane_feature_selector(row)
    # Replace this with code which generates features from the molecule
    # print(features)
